...
### Physical-First QR Architecture (MVP Focus) [DSB-BOP-P2-003]
...
[COLOR:#e63946]**[CB]:** we need to find a product that we can utilise that can do this for us -> need to do a bit of research and align as closely as possible with the flow below[/COLOR]

```mermaid
flowchart TD
    Start([Guest Submits Post]) --> Quarantine[Private Quarantine Bucket]
    Quarantine --> Scan{Server-Side AI Scan<br/>Image & Text}
    
    Scan -->|Clear / No Flags| ActionClear[Status: Published]
    Scan -->|Standard Flag<br/>NSFW, Profanity, Negative| ActionHost[Status: Pending Host Approval]
    Scan -->|Critical Flag<br/>Violence, CSAM, Hate| ActionBypass[Status: Admin Escalation]
    
    ActionClear --> Wall((Live on Public Wall))
    ActionHost --> HostQueue[Host Dashboard<br/>Guest Sees: 'Pending']
    ActionBypass --> AdminQueue[Internal Trust & Safety Queue]
    
    Wall -.-> Report[Post Reported by Guest]
    Report --> Breaker{Anti-Abuse<br/>Circuit Breaker}
    Breaker -->|Rate Limit Hit| Ignored[Logged Neutrally<br/>Visibility Unchanged]
    Breaker -->|Valid Report| Hidden[Hidden Immediately<br/>Host SLA: 14 Days]
```
...
<u>The "How-To" Guide (Property Amenities)</u>
...
<u>Local Hotspots & Hidden Gems</u>
...
<u>The Frictionless Checkout Guide</u>
...
<u>The "Meet the Host" / Property Journey Pin</u>
...
# Part 2: Commercial Strategy & Resource Allocation

## 2.1 Core Focus [DSB-BOP-P3-001]

This product operates strictly in the B2B short-term rental market.

## 2.2 Operational Boundary (Support Level) [DSB-BOP-P3-002]
...
## 2.3 Onboarding & Revenue Safeguards [DSB-BOP-P3-003]
...
## 2.4 Dynamic Billing Architecture [DSB-BOP-P3-004]
...
## 2.5 Marketing Positioning and Booking-Provider Boundary [DSB-BOP-P3-005]
...
# Part 3: Operational Workflows & Communications
...
## 3.1 Commercial Messaging Compliance (Spam Act 2003) [DSB-BOP-P4-001]
...
## 3.2 Host Email Lifecycle [DSB-BOP-P4-002]
...
## 3.3 Lead-Nurturing Cart Abandonment Sequence [DSB-BOP-P4-003]
...
## 3.4 Retention & Engagement Lifecycle [DSB-BOP-P4-004]
...
## 3.5 Internal Admin Lifecycle & Moderation [DSB-BOP-P4-005]
...
## 3.6 Automated Takedown & Reporting State Model [DSB-BOP-P4-006]
...
### Authoritative Reporting State Model [DSB-BOP-P2-005]

All modifications to a post's visibility and its active report status are exclusively governed by a single transactional server endpoint. This strict architecture ensures that client-side actions and direct Firestore writes can never be used to improperly hide, restore, or delete another guest's post.
...
**Reporter limits:** Same reporter plus same post is deduplicated for 24 hours. Reporting 3 distinct posts on one wall within 10 minutes or 5 within 24 hours starts a 24-hour suspected-abuse cooldown. Two internally confirmed malicious-reporting cases within 90 days may create a restriction, linked to a keyed hash of the existing Firebase anonymous/account UID, lasting no more than 90 days. A cleared browser/session cannot be correlated through a newly invented device identity. Reporter restrictions expire automatically.
...
[COLOR:#e63946]**[CB]:** research needs to find a product so this will change depending on product[/COLOR]
...
# Part 4: Future Expansion Roadmap (Post-MVP)
...
## 4.1 Affiliate & Referral Marketing Engine [DSB-BOP-P5-001]
...
## 4.2 Contextual Location-Based Writing Prompts [DSB-BOP-P5-002]
...
## 4.3 Dual-Carousel Tablet Display (Enterprise/Pro Tier) [DSB-BOP-P5-003]
...
## 4.4 Direct Booking Retention Links [DSB-BOP-P5-004]
...
## 4.5 Client-Side Auto-Translation for House Manuals [DSB-BOP-P5-005]
...
# Part 5: Website Architecture & Core UX
...
## 5.1 Site Navigation & Page Flow [DSB-BOP-P6-012]

The following diagram outlines the high-level routing architecture and relationships between public marketing pages, the secure Host dashboard, and the public Guest Wall.

```mermaid
flowchart TD
    %% Public Marketing & Acquisition
    Home([Home / Landing Page]) --> Pricing[Pricing Page]
    Home --> Login[Sign-Up / Log-In]
    Pricing --> Login
    
    %% Host Secure Area
    Login --> Dashboard{Host Control Center}
    Dashboard -->|Select Property| PropPage[Property Page]
    Dashboard -->|Add New| PropPage
    
    %% Public Guest Area
    PropPage -.->|Live Routing URL| Wall([The Guest Wall])
    QR[Printed QR Code] -.->|Guest Scan| Wall
    
    %% Shared / Utility Pages
    Wall --> Upload[Guest Contribution UX]
    Wall --> MicroBrand["Powered by..." Micro-Branding]
    MicroBrand --> Home
    Wall --> Privacy[Public Privacy & Safety]
    Home --> FAQ[Host FAQ]
    Home --> Contact[Host Contact]
```


## 5.2 The Home Page (Main Landing Page) [DSB-BOP-P6-001]
...
## 5.3 The Pricing Page [DSB-BOP-P6-002]
...
## 5.4 Sign-Up / Log-In Page [DSB-BOP-P6-003]
...
## 5.5 Host Control Center (The Dashboard) [DSB-BOP-P6-004]
...
## 5.6 Property Page (Dedicated Management Hub) [DSB-BOP-P6-005]
...
## 5.7 Host FAQ Page [DSB-BOP-P6-006]
...
## 5.8 Host Contact Page [DSB-BOP-P6-007]
...
## 5.9 Public Privacy & Safety Page [DSB-BOP-P6-008]
...
## 5.10 The Guest Wall (Live & Demo Modes) [DSB-BOP-P6-009]
...
### 5.10.1 Layout Control & Content Hierarchy [DSB-BOP-P2-002]
...
### 5.10.2 Pre-Purchase Sandbox & Customization [DSB-BOP-P2-001]
...
## 5.11 "Powered by DigiStayBook" Guest Wall Micro-Branding [DSB-BOP-P6-010]
...
## 5.12 Guest Contribution UX [DSB-BOP-P6-011]
...
# Part 6: Terms and Conditions (Legal Copy)
...
# Part 7: Privacy Policy (Legal Copy)
...
# Part 8: Appendix - Automated Email Copy & Templates
...
### 8.1.2 Property Activation (Asset Delivery) [DSB-BOP-P9-001B]
...
### 8.1.3 Trial Expiry Notice [DSB-BOP-P9-001C]
...
### 8.1.4 Automated Monthly Receipt [DSB-BOP-P9-001D]
...
### 8.1.5 Card Expiring Soon (Pre-Dunning) [DSB-BOP-P9-001E]
...
### 8.1.6 Failed Payment Flow (The Grace Period) [DSB-BOP-P9-001F]
...
### 8.1.7 Service Suspension (The Final Notice) [DSB-BOP-P9-001G]
...
### 8.1.8 Cancellation Confirmation [DSB-BOP-P9-001H]
...
## 8.2 Moderation & Content Safety Emails [DSB-BOP-P9-002]
...
## 8.3 Security & Account System Emails [DSB-BOP-P9-003]
...
## 8.4 Lead-Nurturing Cart Abandonment Sequence [DSB-BOP-P9-004]
...
## 8.5 Retention & Engagement Lifecycle [DSB-BOP-P9-005]
...
## 8.6 Affiliate Marketing Engine (Post-MVP) [DSB-BOP-P9-006]

### 8.6.1 Affiliate Payout Notification [DSB-BOP-P9-006A]
- **Classification:** Transactional
